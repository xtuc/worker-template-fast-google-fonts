...
export class GoogleFontOptimization {
    constructor(request) {
    ...
    isGoogleFontRequest() {
        return (
            ...
            this.url.pathname.startsWith('/fonts.gstatic.com/')
        )
    }

    serveGoogleFont() {
        return fetch(`${this.url.protocol}/${this.url.pathname}`)
    }

    withGoogleFontTransform(rewriter) {
        const request = this.request;
        console.log(request);
        return rewriter.on('link', {
            async element(e) {
                const src =
